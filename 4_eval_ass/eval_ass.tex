...
the the number of contigs \texttt{Newbler} split one read into (in
some cases more than 100 contigs) seems artificially inflated (see
figure \ref{split_454}). If information would correspond to real
isoforms it should be about an order of magnitude lower. This fact
emphasises the need for further processing of the contigs. The maximum
number of read-splits in a given contig and its usefulness will be
...
\figuremacro{read_origin}{Origing of reads}{Reads in the most reliable
  ...
  split by \texttt{Newber} and the intersection reads, which were
...
standard for assesing coverage) can used as to asses the redundancy of
a contig.
...
redundant sequence also found in other contigs.
...
biologicall relevant contigs. All gene-sets mentioned later were thus,
as a matter of routine, controlled for unusual patterns in the contig
meta-data.

\section{Finalising the fullest assembly set}
\label{sec:final-full-assembly}

As additonal measure in order to minimize the amount of sequence with
...
\caption[finalizing the assembly]{\textbf{Final filtering of the
  ...
  contig-category. Only the contigs in bold were not screened from the
  assembly ().}
\label{tab:cov.ex}
\end{center}
\end{table}
