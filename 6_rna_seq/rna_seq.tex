...
euclidean distances (see figure \ref{heat_all} a).
...
\figuremacroW{pca_eel}{Constrained redundancy analysis for host-DE
  ...
  the sex of the worm. (a) Host differences partition the variance in
  samples in like expected for all contigs, the constrained component
  ...
  no significance for this subset of the data.}{0.60}
...
For the lower number of contigs DE between host-species obviously
...
\figuremacro{tGO_pop_BP_classic_10_all}{GO biological process graph
  ...
  the GO-ontology cellular compartment category induced by the top 10
...
\figuremacro{pop_all_heat}{Clustering of expression values for contigs
  ...
  hierarchically clustering. Green indicates expression below the mean,
  red above the mean. Experimental conditions are indicated by black
  bars for groups of samples (columns) below the plot. Presence
